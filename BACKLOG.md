...
| **Metamodel updated** | `rules/metamodel.md`: added Step 7c row + build order section + DAG node + ER entities (`INTERFACE_CONTRACT`, `CLI_SURFACE`, `CLI_COMMAND`) + ID conventions (`BC-NN.CTR-NN`, `CLI-NN`, `CLI-NN.CMD-NN`) + canonical paths (`docs/architecture/interfaces/`) + `arch-cli-surface` supporting skill bullet + maintenance coupling entry. |
