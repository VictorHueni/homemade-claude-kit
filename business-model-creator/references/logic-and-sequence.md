...
| **§3 Filter** | "Of that, what subset actually applies to this mechanic?" | Conflate "Swiss drug market" with "Preismodelle drug market". Different by 10×. |
...
| **§5 Segmentation** | "Distributed across which customers?" | Average across all 37 insurers when 5 of them are 63% of lives. |
| **§6 Scenarios** | "How sensitive is the answer to the assumptions?" | Pretend you know the recovery rate is exactly 50%. |
...
2. **§8 "Key unknowns + validation path"** — every gap you couldn't fill, with the specific way to fill it (one interview, one data pull, one search). Stops the model from rotting into "we don't know but we said CHF 600M anyway."
...
|---|---|---|---|---|
| **Restitution / recovery** | Total relevant spend | Triggerable subset | Recovery / capture rate | Per customer tier |
| **TAM / SAM / SOM** | All addressable orgs × products | Reachable in N years | Closable in Y1-2 | Per segment ARR |
| **Cost-savings / automation** | Current process total cost | Automatable case subset | Auto-approve / straight-through rate | Per customer savings |
| **Subscription / freemium** | Eligible user count | Free-tier converters | Upgrade-to-paid rate | Per seat / per cohort |
| **Outcome-based contracting** | Total addressable patients | Eligible subset | Outcome-success rate | Per provider tier |
...
- **Number of funnel steps** — 3 (TAM/SAM/SOM), 4 (restitution, savings), or 5+. Template suggests 4 but doesn't enforce.
- **§7 value-capture math** — different per product. Recovery SaaS is "% of recovered restitutions"; subscription is "fixed license × installations"; freemium is "free → paid conversion funnel". Template gives the slot, not the math.
...
3. **Draft §5.2 assumptions** before any CHF figure. List every shortcut you're about to take.
...
1. **Misquoted numbers in the pitch deck.** Single point estimates get quoted with false precision. The Scenario matrix forces "CHF 30M-130M depending on recovery rate" instead of "CHF 30M".
2. **Hidden assumption rot.** Six months from now, when a recovery rate has been validated, the §5.2 table tells you exactly which row to flip from ⚠️ to ✅ and which downstream numbers to recompute.
