...
- `com-slide-deck` — HTML slide presentations → `docs/communication/slides/{slug}/` (one folder per deck, named after the presentation in kebab-case); `build.py` inlines the shared `docs/design/tokens.css` (`design-system` skill) before the deck `styles.css` so base palette + typography flow from the project source of truth (config `paths.design_tokens` or auto-detected; falls back to standalone when absent) — the deck adopts the contract token names, including generic semantic state (`--success/--warning/--danger/--info`); kit-domain tokens are consumer-derived (OI-0019)
