...
**Current Increment:** 04 — Resolve Stale `§Open Issues` References and Decide Coverage for Other Business Artefacts
...
| M2: Artefact Normalization | 03-04 | in-progress | Existing templates and discipline docs use one unresolved-work vocabulary and one canonical document-level schema | `rg -n "^## Open Items" arch-research business-process business-research business-workshop business-model-canvas business-capability-map business-value-stream spec-functional-breakdown-structure spec-prd` | No stale `Open Issues` wording remains in targeted artefacts, normalized sections preserve tracker provenance, and `business-process` no longer scaffolds placeholder-only rows | `docs(skills): normalize open-items sections across artefacts` |
