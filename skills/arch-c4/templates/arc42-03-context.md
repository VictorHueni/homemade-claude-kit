...
_TODO_ — describe any domain-specific protocols, message envelopes, or content models that are part of the agreement between {{system_id}} and a partner. Skip if all interactions are plain HTTPS/JSON CRUD.
...
_TODO_ — for each row above, briefly note which business inputs/outputs (from §3.1) flow over which channel. Skip if the mapping is 1:1 obvious.
