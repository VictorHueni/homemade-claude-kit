...
**Process:**
1. **Read project context** — if any exist, read before writing: `docs/VISION.md` (current state), `docs/business/personas/personas.md`, `docs/business/business-model-canvas/`, `docs/business/value-streams/value-streams.md`, `docs/business/objectives/objectives.md`. The vision synthesises these artefacts into a single orientation statement.
...
|---|---|
| **Elevator pitch target** | Personas (`P-NN`) from `docs/business/personas/personas.md` |
| **Problem statement** | Value stream pain index (`VS-N.M` High/Critical stages) from `docs/business/value-streams/value-streams.md` |
| **North Star Metric** | Business Objectives (`OBJ-NN`, `KR-NN.M`) from `docs/business/objectives/objectives.md` — OKRs are the measurable steps toward the North Star |
| **§Linked Artefacts** | BMC Value Propositions (`VP-NN`) from `docs/business/business-model-canvas/` — VP blocks are the commercial expression of the vision per segment |
